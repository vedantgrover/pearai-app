...
export class PreferencesContribution implements IWorkbenchContribution {
	...
	constructor(
		...
		this.settingsListener = this.configurationService.onDidChangeConfiguration(
			(e) => {
				if (
				...
				{},
				{
					createEditorInput: ({
						...
						if (state === WorkbenchState.FOLDER) {
							...
							if (
								isEqual(resource, folders[0].toResource(FOLDER_SETTINGS_PATH))
							) {
								return {
									editor: this.preferencesService.createSplitJsonEditorInput(
						...
						// Multi Folder Workspace Settings File
							...
							for (const folder of folders) {
								if (
									isEqual(resource, folder.toResource(FOLDER_SETTINGS_PATH))
								) {
									return {
										editor: this.preferencesService.createSplitJsonEditorInput(
